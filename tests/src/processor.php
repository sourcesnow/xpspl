...
unittest\suite(function($suite){
    ...
    $suite->test(function($test){
        ...
        $test->instanceof(
            $test->processor->search_signals($signal), 
            new \XPSPL\Queue()
        );
        $test->processor->delete_signal($signal);
        $test->null($test->processor->search_signals($signal));
        // Delete history
        $test->processor->signal($signal, function(){});
        $test->processor->emit($signal);
    ...
    $suite->test(function($test){
        ...
        $queue = $test->processor->search_signals('test');
        $test->count($queue->storage(), 1);
        $test->false($test->processor->has_signal_exhausted('test'));
        $test->processor->remove_process('test', $process);
        $test->count($queue->storage(), 0);
        $test->true($test->processor->has_signal_exhausted('test'));
